...
export function MenuItem(props) {
  ...
  const openUpdateMenu = () => {
    setTitleModal(`Actualizar Menu: ${menu.tipo}`);
    onOpenCloseModal();
  };

  const openDesactiveActivateConfirm = () => {
    setIsDelete(false);
    setConfirmMessage(
      menu.active
        ? `Desactivar Menu: ${menu.tipo}`
        : `Activar Menu: ${menu.tipo}`
    );
    onOpenCloseConfirm();
  };
  ...
  const onActivateDesactivate = async () => {
    try {
      await menuController.updateMenu(accesToken, menu._id, {
        active: !menu.active,
      });
      onReload();
      onOpenCloseConfirm();
  ...
  const openDeleteConfirm = () => {
    setIsDelete(true);
    setConfirmMessage(`Eliminar el Menu: ${menu.tipo}`);
    onOpenCloseConfirm();
  };
    ...
    <>
      <div className="menu-item">
        <div className="menu-item__info">
          <span className="menu-item__info-title">{menu.tipo}</span>
        ...
        <div>
          ...
          <Button
            ...
            onClick={openDesactiveActivateConfirm}
          >
